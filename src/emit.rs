...
pub const EMIT_SCENE: EmitScene = EmitScene::Rain;

/// Default emission rate, in flow-batches per second (one batch = 48 particles,
/// see `dispatch`). 60/s matches the historical "one emit per 1/60 s frame"
/// tuning the rest of the params are calibrated for.
pub const EMIT_BATCHES_PER_SEC: f32 = 60.0;
...
fn rain(physics: &mut Physics, count: u32) {
    ...
    for k in 0..n {
        ...
        if !physics.try_add_object(vec2(x, y), vec2(0.0, speed)) {
            return;
        }
    }
}
